...
void healthyAI::spawn(float startX, float startY, int type, int seed)
{
	switch (type)
	{
	case 0:
		// Healthy sprite
		m_Sprite = Sprite(TextureHolder::GetTexture(resourcePath()+"graphics/healthyAI.png"));
		m_Speed = AI_SPEED;
		break;
	}

	// Prevent AI from bunching
...
Time healthyAI::getLastHitTime() {
	return m_LastHit;
}

bool healthyAI::hit(Time timeHit) {
...
bool healthyAI::isAlive()
{
	return m_Alive;
}

FloatRect healthyAI::getPosition()
{
	return m_Sprite.getGlobalBounds();
}

Sprite healthyAI::getSprite()
{
	return m_Sprite;
}

void healthyAI::update(float elapsedTime, Vector2f playerLocation)
{
	float playerX = playerLocation.x;
	float playerY = playerLocation.y;

	// Update the AI position variables
